...
function wallpaperRoot(ctx: unknown, config: unknown): string | undefined {
  const configured = ((config as { wallpaperDir?: string } | undefined)?.wallpaperDir ?? '').trim();
  if (configured) return resolve(configured);
  const c = ctx as { dshHomePath?: (...segments: string[]) => string };
...
async function serveFile(root: string, rel: string, res: ServerResponse): Promise<void> {
...
/** Wallpaper file extensions accepted by the upload endpoint. */
...
/** Upload size cap (bytes) — wallpaper videos are short loops, 200MB is plenty. */
 ...
 */
async function uploadFile(req: IncomingMessage, res: ServerResponse, root: string): Promise<void> {
  if (req.method !== 'POST') {
    res.writeHead(405);
    res.end();
    return;
  ...
  const safeBase = basename(original)
    .replace(/[^\w.\-]+/g, '_')
    .replace(/^\.+/, '')
    .slice(0, 120);
  ...
  res.end(JSON.stringify({ name: stored }));
}

async function proxyUrl(req: IncomingMessage, res: ServerResponse): Promise<void> {
...
export function apply(ctx: unknown, config: unknown): void {
  const c = ctx as {
    inject(services: string[], fn: (sub: unknown) => void): void;
  ...
  c.inject(['webServer'], (httpCtx) => {
    const webServer = (httpCtx as { webServer: { register(route: unknown): () => void } }).webServer;
    c.effect(
      () =>
        webServer.register({
          kind: 'prefix',
