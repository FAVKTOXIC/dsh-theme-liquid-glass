...
/* Composer card toolbar buttons (commands +, attachments, etc.): ensure their
   icon color is visible against the frosted glass surface. The "+" command
...
body.dsh-lg-on [data-composer-card] button[aria-label="命令"]:hover,
body.dsh-lg-on [data-composer-card] button[aria-label*="Command" i]:hover {
  color: var(--dsw-alias-label-primary) !important;
  background: color-mix(in srgb, var(--dsh-lg-tint, #ffffff) 28%, transparent) !important;
}
body[data-ds-dark-theme].dsh-lg-on [data-composer-card] button[aria-label="命令"],
body[data-ds-dark-theme].dsh-lg-on [data-composer-card] button[aria-label*="Command" i] {
  color: var(--dsw-alias-label-primary) !important;
  background: color-mix(in srgb, var(--dsh-lg-tint-dark, #333333) 30%, transparent) !important;
}
body[data-ds-dark-theme].dsh-lg-on [data-composer-card] button[aria-label="命令"]:hover,
body[data-ds-dark-theme].dsh-lg-on [data-composer-card] button[aria-label*="Command" i]:hover {
  background: color-mix(in srgb, var(--dsh-lg-tint-dark, #333333) 45%, transparent) !important;
}
  ...
  client.effect(() => {
    const applyIcon = (): void => {
      ...
      const cell = Array.from(dialog.querySelectorAll<HTMLButtonElement>('button')).find(
        (b) => b.textContent?.trim() === OUR_LABEL,
      );
      if (!cell) return;
      ...
      iconSlot.innerHTML =
        '<svg class="lg-watermark" xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 22a7 7 0 0 0 7-7c0-2-1-3.9-3-5.5s-3.5-4-4-6.5c-.5 2.5-2 4.9-4 6.5C6 11.1 5 13 5 15a7 7 0 0 0 7 7z"/></svg>';
    };
    applyIcon();
